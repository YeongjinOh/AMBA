...
var form = div().appendTo(root).displayBlock().size('300px', '340px').position('relative').margin('auto').top('50%')
    .transform('translate(0%, -50%)').color('#4c00e6');
...
div().appendTo(form).displayBlock().size('270px', '20px').margin('auto').marginTop('20px').color('#4c00e6').text('Email')
    .fontSize('15px').fontColor('white').disableSelection().cursorDefault();
...
div().appendTo(form).displayBlock().size('270px', '20px').margin('auto').marginTop('15px').color('#4c00e6').text('Password')
    .fontSize('15px').fontColor('white').disableSelection().cursorDefault();
...
div().appendTo(form).displayBlock().size('270px', '40px').margin('auto').marginTop('30px').color('#1a53ff').borderRadius(5).boxShadow('5px 5px 5px black')
    .text('Sign In').textAlignCenter().fontSize('25px').fontColor('white').fontBold().disableSelection().cursorPointer().hover(function (dv) {
    dv.fontColor('black');
},  function (dv) {
    dv.fontColor('white');
}).click( function () {
...
div().appendTo(form).displayBlock().size('50px', '20px').float('right').marginRight('15px').marginTop('10px').textAlignRight()
    .color('#4c00e6').text('Sign Up').fontSize('13px').fontColor('white').fontBold().disableSelection().cursorPointer().hover(function (dv) {
    dv.fontColor('black');
},  function (dv) {
    dv.fontColor('white');
}).click( function () {
